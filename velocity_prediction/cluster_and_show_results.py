# import os
# from sqp_optimal_speed_track import *
...
if __name__ == '__main__':
    ...
    # calculate ori 
    for i in range(len(res_rec)):
        energy_ori = energy_cal(v_test[i])
    ...
    with open('result_cluster.pickle','rb') as f:
        [result_cluster] = pickle.load(f)     # v_test : m/s  distance : m
    ratio = [result_cluster['cluster4']['opt4']['energy'][i] / result_cluster['cluster4']['ori4']['energy'][i] - 1 for i in range(0,len(result_cluster['cluster4']['ori4']['energy']))]
    ratio_opt = []
    for i in ratio:
        if i<0:
            ratio_opt.append(i)
